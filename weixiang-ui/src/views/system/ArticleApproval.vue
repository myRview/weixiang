<template>
  <div class="article-approval">
    ...
    <el-dialog
      ...
      width="500px"
      :title="dialogType === 'approve' ? '审核通过' : '审核驳回'"
    >
      <el-form :model="formData" label-width="80px">
        <el-form-item v-if="dialogType === 'reject'" label="驳回原因">
          <el-input
            type="textarea"
        ...
        <el-form-item v-if="dialogType === 'approve'">
          <div class="approve-tip">确定要审核通过这篇文章吗？</div>
        </el-form-item>
      </el-form>
      <template #footer>
...
import { selectArticlePage, auditArticle } from "@/api/wenzhangguanli";
...
const selectPage = async () => {
  try {
    ...
    const res = await selectArticlePage(searchParam.value);
...
const formData = ref<{ id?: number; auditReason?: string }>({});
const currentArticle = ref<API.ArticleVO>();

// 审核通过
const handleApprove = (row: API.ArticleVO) => {
...
// 审核驳回
const handleReject = (row: API.ArticleVO) => {
  currentArticle.value = row;
  dialogType.value = "reject";
  formData.value = { id: row.id, auditReason: "" };
  dialogVisible.value = true;
};
...
const handleDialogConfirm = async () => {
  ...
  try {
    const params: API.ArticleAuditVO = {
      articleId: currentArticle.value.id,
      auditStatus: dialogType.value === "approve" ? 1 : 2,
      auditReason:
        dialogType.value === "reject" ? formData.value.auditReason : undefined,
    };

    const res = await auditArticle(params);
    if (res.data.code === 200) {
      ElMessage.success(
        dialogType.value === "approve" ? "审核通过成功" : "审核驳回成功"
      );
      dialogVisible.value = false;
      selectPage();
    } else {
      ElMessage.error(res.data.message || "审核操作失败");
    }
  } catch (err) {
